        ...
        {(pending || result || error) && <div className="execution" aria-live="polite">
          ...
          {result && <div className={`result ${allowed ? "allow" : "deny"}`} role="status">
            ...
            <dl>
              ...
              <div><dt>{result.execution_mode === "hosted_walkthrough" ? "Illustrative reason" : "Receiver reason"}</dt><dd><code>{result.reason}</code></dd></div>
              <div><dt>{result.execution_mode === "hosted_walkthrough" ? "Expected enforcement layer" : "Decided by"}</dt><dd>{deciderLabels[result.decided_by] ?? result.decided_by}</dd></div>
            ...
            <details><summary>Technical evidence and timings</summary><p>{result.execution_mode === "hosted_walkthrough" ? "Illustrated enforcement layer" : "Deciding layer"} <code>{result.decided_by}</code> · {result.execution_mode === "hosted_walkthrough" ? "Illustrative status" : "Ratify verification status"} <code>{result.verification_status ?? "not reached"}</code> · Audience <code>{result.delegation_audience}</code> · Delegation issued {new Date(result.delegation_issued_at * 1000).toLocaleString()} · Correlation <code>{result.correlation_id}</code> · {result.execution_mode === "hosted_walkthrough" ? "Fixture generated" : "Executed"} {new Date(result.timestamp).toLocaleString()} · No keys, proof material, or private identifiers are displayed.</p><div className="timings"><b>{result.execution_mode === "hosted_walkthrough" ? "Request timings (fixture response)" : "Observed timings"}</b><span>Browser total: {browserDuration ?? "—"} ms</span><span>Proxy upstream: {result.upstream_duration_ms} ms</span><span>Agent authority path: {result.interceptor_duration_ms ?? "not emitted"} ms</span><span>Challenge: {result.challenge_duration_ms ?? "not reached"} ms</span><span>Proof upload: {result.proof_upload_duration_ms ?? "not reached"} ms</span><span>Dispatch: {result.dispatch_duration_ms ?? "not reached"} ms</span><span>Proof construction: {result.proof_build_duration_ms ?? "not reached"} ms</span><span>Receiver verification: {result.verification_duration_ms ?? "not reached"} ms</span></div></details>
          </div>}
        </div>}
      </section>
